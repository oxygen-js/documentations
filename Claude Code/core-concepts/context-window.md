# Изучение окна контекста

> Интерактивная симуляция того, как окно контекста Claude Code заполняется в ходе сессии. Посмотрите, что загружается автоматически, во что обходится каждое чтение файла и когда срабатывают правила и хуки.
...
Окно контекста Claude Code содержит всё, что Claude знает о вашей сессии: ваши инструкции, файлы, которые он читает, его собственные ответы и контент, который никогда не появляется в вашем терминале. Хронология ниже проигрывает полную сессию от запуска до сжатия: что загружается до того, как вы что-либо введёте, что добавляет каждое чтение файла, правило и хук по мере работы Claude, и как субагент удерживает крупные чтения вне вашего контекста. Смотрите [текстовый разбор](#what-the-timeline-shows) для того же содержания в виде списка.

<ContextWindow />

## Что показывает хронология

Сессия проходит через реалистичный сценарий с репрезентативными значениями числа токенов:

* **До того, как вы что-либо введёте**: CLAUDE.md, автопамять, имена MCP-инструментов и описания навыков — всё это загружается в контекст. Ваша собственная конфигурация может добавить сюда что-то ещё, например [стиль вывода](https://code.claude.com/docs/en/output-styles) или текст из [`--append-system-prompt`](https://code.claude.com/docs/en/cli-reference), которые оба попадают в системный промпт одинаковым образом.
* **По мере работы Claude**: каждое чтение файла добавляется к контексту, [правила с привязкой к путям](https://code.claude.com/docs/en/memory#path-specific-rules) загружаются автоматически вместе с подходящими файлами, а [хук PostToolUse](https://code.claude.com/docs/en/hooks-guide) срабатывает после каждого редактирования.
* **Уточняющий промпт**: [субагент](https://code.claude.com/docs/en/sub-agents) выполняет исследование в собственном отдельном окне контекста, поэтому крупные чтения файлов остаются вне вашего. Обратно возвращаются только сводка и небольшой завершающий блок метаданных.
* **В конце**: `/compact` заменяет разговор структурированной сводкой. Большая часть стартового контента перезагружается автоматически; таблица ниже показывает, что происходит с каждым механизмом.

## Что переживает сжатие

Когда длинная сессия сжимается, Claude Code резюмирует историю разговора, чтобы уместить её в окно контекста. Что произойдёт с вашими инструкциями, зависит от того, как они были загружены:

| Механизм                                     | После сжатия                                                                                              |
| :------------------------------------------- | :------------------------------------------------------------------------------------------------------- |
| Системный промпт и стиль вывода              | Без изменений; не является частью истории сообщений                                                       |
| Корневой CLAUDE.md проекта и правила без привязки | Повторно вставляются с диска                                                                          |
| Автопамять                                   | Повторно вставляется с диска                                                                              |
| Правила с frontmatter `paths:`               | Теряются, пока снова не будет прочитан подходящий файл                                                    |
| Вложенные CLAUDE.md в подкаталогах           | Теряются, пока снова не будет прочитан файл в этом подкаталоге                                             |
| Тела вызванных навыков                       | Повторно вставляются с ограничением 5 000 токенов на навык и 25 000 токенов суммарно; самые старые отбрасываются первыми |
| Хуки                                         | Неприменимо; хуки выполняются как код, а не как контекст                                                  |

Правила с привязкой к путям и вложенные файлы CLAUDE.md загружаются в историю сообщений, когда прочитан их триггерный файл, поэтому сжатие резюмирует их вместе со всем остальным. Они перезагружаются в следующий раз, когда Claude прочитает подходящий файл. Если правило должно сохраняться при сжатии, уберите frontmatter `paths:` или перенесите его в корневой CLAUDE.md проекта.

Тела навыков повторно вставляются после сжатия, но крупные навыки усекаются, чтобы уместиться в лимит на навык, а самые старые вызванные навыки отбрасываются, как только превышен общий бюджет. Усечение сохраняет начало файла, поэтому размещайте самые важные инструкции ближе к началу `SKILL.md`.

## Когда контекст заполняется

Claude Code сжимает контекст автоматически по мере приближения к лимиту, поэтому заполненное окно контекста не завершает вашу сессию. Автоматический проход работает так же, как шаг `/compact` в хронологии. Смотрите [Когда контекст заполняется](https://code.claude.com/docs/en/how-claude-code-works#when-context-fills-up), чтобы узнать, что при этом сохраняется.

Вы также можете действовать до того, как запустится автоматический проход:

* **Сжатие с фокусом**: запустите `/compact` с инструкциями, например `/compact focus on the auth bug fix`, перед началом длинной новой задачи. Сводка сохранит то, что вы выберете, вместо того, что автоматический проход посчитает важным.
* **Очистка между задачами**: запускайте `/clear` при переключении на несвязанную работу. Старый разговор вытесняет файлы, которые понадобятся дальше, и расходует токены в каждом сообщении.
* **Делегируйте крупные чтения**: отправляйте исследование [субагенту](https://code.claude.com/docs/en/sub-agents), чтобы содержимое файлов оставалось в его окне контекста, а не в вашем.

Если вам нужно окно побольше, а не разговор поменьше, Fable 5, Sonnet 5, Opus 4.6 и новее, а также Sonnet 4.6 поддерживают окно контекста на 1 миллион токенов. Смотрите [Расширенный контекст](https://code.claude.com/docs/en/model-config#extended-context) для сведений о доступности по тарифам и о том, как выбрать вариант модели `[1m]`. Sonnet 5 работает на 1 млн без варианта `[1m]`, который нужно было бы выбирать; смотрите [Окно контекста Sonnet 5](https://code.claude.com/docs/en/model-config#sonnet-5-context-window) для его порогов автосжатия и исключения для шлюза LLM. Сжатие работает так же при большем лимите.

## Проверьте свою сессию

Визуализация использует репрезентативные числа. Чтобы увидеть ваше фактическое использование контекста в любой момент, запустите `/context` для получения актуального разбора по категориям с предложениями по оптимизации. Запустите `/memory`, чтобы проверить, какие файлы CLAUDE.md и автопамяти загрузились при запуске.

## Связанные ресурсы

Для более глубокого освещения возможностей, показанных в хронологии, смотрите эти страницы:

* [Расширение Claude Code](https://code.claude.com/docs/en/features-overview): когда использовать CLAUDE.md, навыки, правила, хуки или MCP
* [Хранение инструкций и памяти](https://code.claude.com/docs/en/memory): иерархия CLAUDE.md и автопамять
* [Субагенты](https://code.claude.com/docs/en/sub-agents): делегирование исследования отдельному окну контекста
* [Лучшие практики](https://code.claude.com/docs/en/best-practices): управление контекстом как вашим главным ограничением
* [Кэширование промптов](https://code.claude.com/docs/en/prompt-caching): какие действия аннулируют кэшированный префикс
* [Снижение расхода токенов](https://code.claude.com/docs/en/costs#reduce-token-usage): стратегии для поддержания низкого использования контекста

---

> _Перевод официальной документации Claude Code (раздел «Основные концепции»). Источник: https://code.claude.com/docs/en/context-window. © Anthropic._
